...
- Verification of expected requests have been performed
...
// Verify at least one request to a specific endpoint with any query params
hoverflyRule.verify(service(matches("*.flight.*")).get("/api/bookings").anyQueryParams(), atLeastOnce());

// Verify an external service/dependency is never called
hoverflyRule.verifyNever(service(matches("*.flight.*")));
